/*
 * $Id: bmeipc.c 1948 2009-06-12 11:08:31Z spiiroin $
 *
 * Library for communicating with BME.
 */
 ...
 */
static void settimeout(struct timeval *timeout, int msec)
{
  struct timeval s =
  {
    .tv_sec  = (msec / 1000),
 ...
 */
static int msecsto(const struct timeval *timeout)
{
  struct timeval t;
  int msec = 0;
 ...
 * @va:    appropriate arguments for the format string
 */
static void log_message_va(int level, const char *fmt, va_list va)
{
  int saved = errno;
  char exe[64];
  char msg[256];
  ...
  switch(level) {
  case LOG_DEBUG:   lev = "debug";   break;
  case LOG_INFO:    lev = "info";    break;
  case LOG_NOTICE:  lev = "notice";  break;
  case LOG_WARNING: lev = "warning"; break;
  case LOG_ERR:     lev = "error";   break;
  }

  /* Get process name from /proc */
  n = readlink("/proc/self/exe", exe, sizeof exe - 1);
  exe[(n > 0 && n < sizeof exe) ? n : 0] = 0;
  
  /* Restore errno for '%m' in case readlink failed */
  errno = saved;
  ...
  for( s = d = e = (unsigned char *)msg; *s; ++s )
  {
    if( *s <= ' ' ) *d++ = ' ';
    else            *d++ = *s, e = d;
  }
  *e = 0;
  ...
  fprintf(stderr, "%s: BMEIPC: %s: %s\n", basename(exe), lev, msg);
  fflush(stderr);
  errno = saved;
}
#endif
 ...
 */
static int bme_bytes_read(int fd, void *data, int size)
{
 ...
 *
 */
static int bme_header_read(int fd)
{
  bmeipc_header head;
...
{
    tBMEmsgGeneric gm = { BME_SYSMSG_GET_PID, 0 };
